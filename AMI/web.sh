...
sudo openssl req -newkey rsa:2048 -nodes -keyout /etc/pki/tls/private/opsmen.key -x509 -days 365 -out /etc/pki/tls/certs/opsmen.crt \
-subj "/C=UK/ST=London/L=London/O=StegHub/OU=DevOps/CN=$(curl -s http://169.254.169.254/latest/meta-data/local-hostname)"

sudo sed -i 's/localhost.crt/opsmen.crt/g'  /etc/httpd/conf.d/ssl.conf

sudo sed -i 's/localhost.key/opsmen.key/g'  /etc/httpd/conf.d/ssl.conf